      ...
      <div style={{ top: '-0.5px' }} className='sticky flex w-full z-40 flex-col'>
                            ...
                            page.subPage.length
                              ? (
                                <div ref={subPageRef} className='flex flex-col gap-2' style={{ maxHeight: `${subPage}px`, overflow: 'hidden', transition: 'max-height 0.5s' }}>
                                  {
                                    page.subPage.map(subPage => <Link href={subPage.slug!} onClick={() => {
                                      setMenu('-ml-[350px]')
                                      setTimeout(() => {
                                        setIndex('hidden')
                                      }, 500)
        ...
        <div className={`${subPages !== -1 ? 'top-0' : '-mt-[130px]'} ${style.design === 'Borde' ? 'border-b' : ''} transition-all duration-500 z-30 w-full p-6 bg-white flex absolute h-32`} style={{ top: '52px', boxShadow: style.design === 'Sombreado' ? '0px 0px 10px 0px #11111115' : '' }} onMouseEnter={() => setSubPages(subPages)} onMouseLeave={() => setSubPages(-1)}>
          <div className='m-auto flex gap-6'>
            {
              subPages !== -1 ? design.pages[subPages].subPage?.map(subPage => <Link href={subPage.slug!} className='text-lg' onClick={() => setSubPages(-1)}>{subPage.page}</Link>) : ''
            }
          </div>
        </div>
      </div>
