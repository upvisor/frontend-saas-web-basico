...
export default function CategoryCard({ category, title }: { category: ICategory, title: string }) {
  ...
  return (
    <Link onMouseEnter={() => setMouse(true)} onMouseLeave={() => setMouse(false)} href={`/tienda/${category.slug}`} key={category._id} className="flex flex-row gap-4 w-full lg:flex-col">
