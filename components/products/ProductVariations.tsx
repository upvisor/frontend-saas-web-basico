            ...
            if (variation.stock > 0) {
              return (
                <div key={variation.variation}>
                  ...
                  }} className={`w-20 h-20 transition-colors duration-150 border rounded-lg p-1 cursor-pointer hover:border-button ${!tempCartProduct.variation?.subVariation ? tempCartProduct.variation?.variation === variation.variation ? 'border-button dark:border-white' : 'dark:border-neutral-700 hover:dark:border-white' : tempCartProduct.variation?.variation === variation.variation && tempCartProduct.variation?.subVariation === variation.subVariation ? 'border-button' : 'dark:border-neutral-700 hover:dark:border-button'}`} />
                </div>
              )
            } else {
              return (
